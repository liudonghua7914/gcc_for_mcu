...
ASRC = $(CMSISDEVDIR)/startup_LPC17xx.s
...
EXTRAINCDIRS = $(CMSISDEVDIR) $(CMSISCOREDIR) $(TARGETCOMMINC) $(INTERFACE)/Inc $(NXPDRVINCDIR) 
...
# s = -Os enables all -O2 optimizations that do not typically increase code size
...
#LDFLAGS +=-T$(LINKERSCRIPTPATH)/$(CMSISDEVDIR)_ram.ld
LDFLAGS +=-T$(CMSISDEVDIR)/$(CHIP)_ram.ld
else 
LDFLAGS +=-T$(CMSISDEVDIR)/$(CHIP)_flash.ld
endif
...
# English
MSG_LDH	= -------LDH Start-------
MSG_HEX	= LDH build hex
MSG_BIN	= LDH build bin
MSG_ELF = LDH build elf
MSG_LSS = LDH build lss
MSG_SYM = LDH build sym
